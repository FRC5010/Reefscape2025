...
public class SwerveDrive {
   ...
   */
  private final DoublePublisher rawIMUPublisher = NetworkTableInstance.getDefault()
      .getTable(
          "SmartDashboard")
      .getDoubleTopic(
          "swerve/imu/raw")
   ...
   */
  private final DoublePublisher adjustedIMUPublisher = NetworkTableInstance.getDefault()
      .getTable(
          "SmartDashboard")
      .getDoubleTopic(
          "swerve/imu/adjusted")
  ...
  /**
   * Correct chassis velocity in
   * {@link SwerveDrive#setChassisSpeeds(ChassisSpeeds chassisSpeeds)} (auto)
   * using 254's
  ...
  /**
   * Correct for skew that scales with angular velocity in
   * {@link SwerveDrive#setChassisSpeeds(ChassisSpeeds chassisSpeeds)} during
   * auto.
   */
  public boolean autonomousAngularVelocityCorrection = false;
  /**
   * Angular Velocity Correction Coefficent (expected values between -0.15 and
   * 0.15).
   */
  ...
  /**
   * Amount of seconds the duration of the timestep the speeds should be applied
   * for.
   */
   ...
   * @param maxSpeedMPS      Maximum speed of the robot in meters per second,
   *                         remember to use
   *                         {@link Units#feetToMeters(double)} if you have feet
   *                         per second!
  ...
  /**
   * Tertiary method of controlling the drive base given velocity in both field
   * oriented and robot oriented at the same
   * time. The inputs are added together so this is not intended to be used to
   ...
   * for the wheel velocities. Applies
  ...
  public void drive(ChassisSpeeds robotRelativeVelocity, boolean isOpenLoop, Translation2d centerOfRotationMeters) {
    ...
    if (headingCorrection) {
      if (Math.abs(robotRelativeVelocity.omegaRadiansPerSecond) < HEADING_CORRECTION_DEADBAND
        ...
        robotRelativeVelocity.omegaRadiansPerSecond = swerveController
            .headingCalculate(getOdometryHeading().getRadians(), lastHeadingRadians);
      } else {
   ...
   *
   * @param maxTranslationalSpeedMetersPerSecond  The allowable max speed that
   *                                              your robot should reach while
   *                                              translating
   *                                              in meters per second.
   * @param maxRotationalVelocityRadiansPerSecond The allowable max speed the
   *                                              robot should reach while
   *                                              rotating in
   *                                              radians per second.
   */
   ...
   * {@link SwerveDrive#maxChassisSpeedMPS} whichever is the lower limit on the
   * robot's speed.
   *
   * @return Minimum speed in meters/second of physically attainable and user
   *         allowable limits.
   */
   ...
   *
   * @return {@link LinearVelocity} representing the maximum drive speed of a
   *         module.
   */
   ...
   * {@link SwerveControllerConfiguration#maxAngularVelocity}, whichever is the
   * lower limit on the robot's speed.
   *
   * @return Minimum angular velocity in radians per second of physically
   *         attainable and user allowable limits.
   */
   ...
   *
   * @param desiredStates       A list of SwerveModuleStates to send to the
   *                            modules.
   * @param desiredChassisSpeed The desired chassis speeds to set the robot to
   *                            achieve.
   * @param isOpenLoop          Whether to use closed-loop velocity control. Set
   *                            to true to disable closed-loop.
   */
   ...
   * auto paths. Does not allow for usage of
   ...
   * @param feedforwards     Feedforward forces generated by set-point
   *                              generator
   */
  public void drive(ChassisSpeeds robotRelativeVelocity, SwerveModuleState[] states, DriveFeedforwards feedforwards) {
    SwerveDriveTelemetry.startCtrlCycle();
    if (SwerveDriveTelemetry.verbosity.ordinal() >= TelemetryVerbosity.LOW.ordinal()) {
      SwerveDriveTelemetry.desiredChassisSpeedsObj = robotRelativeVelocity;
    }
    for (SwerveModule module : swerveModules) {
      ...
      // from the module configuration, obtain necessary information to calculate
      // feed-forward
      // Warning: Will not work well if motor is not what we are expecting.
      // Warning: Should replace module.getDriveMotor().simMotor with expected motor
      // type first.
      ...
      double feedforwardVoltage = driveMotorModel.getVoltage(
          ...
          feedforwards.linearForces()[module.moduleNumber].in(Newtons) * wheelRadiusMeters / driveGearRatio,
      ...
      module.setDesiredState(
          ...
          false,
          feedforwards.accelerations()[module.moduleNumber],
          feedforwards.torqueCurrents()[module.moduleNumber]
      );
    }
  }
  ...
  /**
   * Gets the maple-sim drivetrain simulation instance This is used to add intake
   * simulation / launch game pieces from
   * the robot
   *
   * @return an optional maple-sim {@link SwerveDriveSimulation} object, or
   *         {@link Optional#empty()} when calling from a
   *         real robot
   */
   ...
   *
   * @return an {@link Optional} {@link Pose2d}, representing the drivetrain pose
   *         during simulation, or an empty
  ...
  public ChassisSpeeds getFieldVelocity() {
    ...
    // negating the
    // angle given as the robot angle reverses the direction of rotation, and the
  ...
  /**
   * Enable auto synchronization for encoders during a match. This will only occur
   * when the modules are not moving for a
   * few seconds.
   *
  ...
  public void updateOdometry() {
    ...
    try {
      ...
      if (lastPoseEstimate != null) {
        ...
        if (false && distance > 0.1 && DriverStation.isEnabled() && DriverStation.isAutonomous()
            && ((DriverStation.getMatchTime() > 12.0 && DriverStation.isFMSAttached()) || 
            (DriverStation.getMatchTime() < 3.0 && !DriverStation.isFMSAttached()))) {
          DriverStation.reportError("Pose error: Distance " + distance + "  Yaw " + yaw + " Last Pose "
              + lastPoseEstimate + " to " + currentPose, false);
          Arrays.stream(getModulePositions())
              .forEach(it -> DriverStation.reportError("Pose error: Module " + it, false));
   ...
   *                                 matrix is in the form [x, y, theta],
  ...
  /**
   * Get the {@link SwerveModule}'s as a {@link HashMap} where the key is the
   * swerve module configuration name.
   *
  ...
  /**
   * Set module optimization to be utilized or not. Sometimes it is desirable to
  ...
  /**
   * Enable auto-centering module wheels. This has a side effect of causing some
   * jitter to the robot when a PID is not
   * tuned perfectly. This function is a wrapper for
   * {@link SwerveModule#setAntiJitter(boolean)} to perform
   * auto-centering.
   *
   ...
   * {@link swervelib.parser.SwerveModuleConfiguration#useCosineCompensator} for
   * all
   * {@link SwerveModule}'s in the swerve drive. The cosine compensator will slow
   * down or speed up modules that are
  ...
  /**
   * Sets the Chassis discretization seconds as well as enableing/disabling the
   * Chassis velocity correction in teleop
   *
   * @param enable    Enable chassis velocity correction, which will use
   *                  {@link ChassisSpeeds#discretize(ChassisSpeeds, double)}}
   *                  with the following.
   * @param dtSeconds The duration of the timestep the speeds should be applied
   *                  for.
   */
  ...
  /**
   * Enables angular velocity skew correction in teleop and/or autonomous and sets
   * the angular velocity coefficient for
   * both modes
   *
   * @param useInTeleop          Enables angular velocity correction in teleop.
   * @param useInAuto            Enables angular velocity correction in
   *                             autonomous.
   * @param angularVelocityCoeff The angular velocity coefficient. Expected values
   *                             between -0.15 to 0.15. Start with a
   *                             value of 0.1, test in teleop. When enabling for
   *                             the first time if the skew is
   *                             significantly worse try inverting the value. Tune
   *                             by moving in a straight line while
   *                             rotating. Testing is best done with angular
   *                             velocity controls on the right stick.
   *                             Change the value until you are visually happy
   *                             with the skew. Ensure your tune works
   *                             with different translational and rotational
   *                             magnitudes. If this reduces skew in teleop,
   *                             it may improve auto.
   */
   ...
   *
   * @param robotRelativeVelocity The chassis speeds to set the robot to achieve.
   * @return {@link ChassisSpeeds} of the robot after angular velocity skew
   *         correction.
   */
   ...
   *
   * @param robotRelativeVelocity            The chassis speeds to set the robot
   *                                         to achieve.
   * @param uesChassisDiscretize             Correct chassis velocity using 254's
   *                                         correction.
   * @param useAngularVelocitySkewCorrection Use the robot's angular velocity to
   *                                         correct for skew.
  ...
  /**
   * Convert a {@link ChassisSpeeds} to {@link SwerveModuleState[]} for use
   * elsewhere.
   *
   * @param robotRelativeVelocity {@link ChassisSpeeds} velocity to use.
   * @param optimize              Perform chassis velocity correction or angular
   *                              velocity correction.
