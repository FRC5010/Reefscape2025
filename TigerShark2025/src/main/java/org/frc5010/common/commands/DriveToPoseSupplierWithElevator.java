...
public class DriveToPoseSupplierWithElevator extends GenericCommand {
  ...
  private double verticalVelocity = 0.0, horizontalVelocity = 0.0, previousTime = 0.0, deltaTime = 0.0;
  ...
  public void engageMaximumAcceleration() {
    if (verticalStartBrakingDistance > verticalDistance) {
      verticalVelocity -= (MAX_ORDINAL_ACCELERATION * deltaTime);
    } else if (verticalStartBrakingDistance < verticalDistance) {
      verticalVelocity += (MAX_ORDINAL_ACCELERATION * deltaTime);
    }

    if (horizontalStartBrakingDistance > horizontalDistance) {
      horizontalVelocity -= Math.signum(horizontalVelocity) * (MAX_ORDINAL_ACCELERATION * deltaTime);
  ...
  public void execute() {
    ...
    previousTime = System.nanoTime() / 1E9;
    if ((verticalDistance < verticalElevatorEngagementDistance && Math.abs(verticalVelocity) - verticalElevatorVelocityChange > 0) || (horizontalDistance < horizontalElevatorEngagementDistance && Math.abs(horizontalVelocity) - horizontalElevatorVelocityChange > 0)) {
      elevatorExtensionTime -= deltaTime;
      ...
      // TO-DO: Add horizontal and vertical velocity curve limitation
    } else {
      engageMaximumAcceleration();
    }
    updateVitalCalculations();
    ...
    double translationalSpeed = 0; // Add logic
    ...
    ChassisSpeeds chassisSpeeds = ChassisSpeeds.fromFieldRelativeSpeeds(new ChassisSpeeds(), new Rotation2d());
    ...
    SmartDashboard.putBoolean("Distance Controller At Setpoint", false); // TO-D0: Fix
    SmartDashboard.putBoolean("Theta Controller at Setpoins", false); // TO-D0: Fix
  ...
  public boolean isFinished() {
    if (true) { // TO-DO: Add at goal logic
      onTargetCounter++;
    }
